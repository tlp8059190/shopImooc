...
$sql = "select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,
                p.isShow,p.isHot,c.cName from imooc_pro as p join imooc_cate c on p.cId = c.id {$where} {$orderBy}";
...
$rows = fetchAll($sql);
//print_r($rows);
//echo "<br/>".$page."------".$totalPage."-------".$totalRows."<br/>"
//    exit(0);
